...
def test_anglr_within():
    ...
    assert angle.angle_within(QUARTER_PI, 0) == True
    angle = Angle(-0.8)
    assert angle == TWO_PI - Angle(0.8)
    assert angle.angle_within(0, TWO_PI - QUARTER_PI) == True

def test_anglr_between_clockwise():
    start_angle = Angle(0)
    target_angle = QUARTER_PI
    assert start_angle.angle_between_cw(target_angle) == QUARTER_PI
    ...
    assert start_angle.angle_between_cw(target_angle) == Angle(80, "degrees")

    start_angle = Angle(10, "degrees")
    target_angle = TWO_PI
    assert start_angle.angle_between_cw(target_angle) == Angle(350, "degrees")
    ...
    start_angle = Angle(315, "degrees")
    target_angle = QUARTER_PI
    assert start_angle.angle_between_cw(target_angle) == HALF_PI

def test_anglr_between_counter_clockwise():
    start_angle = Angle(0)
    target_angle = QUARTER_PI
    assert start_angle.angle_between_ccw(target_angle) == Angle(360 - 45, "degrees")

    start_angle = Angle(45, "degrees")
    target_angle = Angle(-45, "degrees")
    assert start_angle.angle_between_ccw(target_angle) == HALF_PI

    start_angle = TWO_PI
    target_angle = Angle(10, "degrees")
    assert start_angle.angle_between_ccw(target_angle) == Angle(350, "degrees")

    start_angle = Angle(180, "degrees")
    target_angle = Angle(135, "degrees")
    assert start_angle.angle_between_ccw(target_angle) == QUARTER_PI
